...
animals<-read_parquet(here::here('data/intermediate_files/animals.parquet'))%>% #each row is an animal
  fxn_standardze_cow_breed(default_pct = 0.03)
...
animal_event_existance<-read_parquet(here::here('data/intermediate_files/events_all_columns.parquet'))%>% #add a few parameters to animal
  ...
  group_by(id_animal)%>%
  summarize(location_lact_list = paste0(sort(unique(location_event)), collapse = ','), 
...
for (i in seq_along(calendar$date_calendar)){
  ...
  df2<-df%>%
    group_by(location_lact_list, lact_group_basic, breed_simple, date_ref) |>  #add custom grouping variable here
    summarize(ct_animals = sum(eligible))%>%
    ungroup() |> 
    rename(`Lactation Group` = lact_group_basic)
  
  df3<-df%>%
    group_by(location_lact_list, lact_group, breed_simple, date_ref) |> #add custom grouping variable here
    summarize(ct_animals = sum(eligible))%>%
    ungroup()|> 
    rename(`Lactation Group` = lact_group)
  
  df4<-df%>%
    group_by(location_lact_list, lact_group_5, breed_simple, date_ref) |> #add custom grouping variable here
    summarize(ct_animals = sum(eligible))%>%
    ungroup()|> 
...
ggplot(deno_dataframe)+
  ...
  ylim(c(0, NA))+
  facet_grid(location_lact_list~breed_simple)+
  theme_bw()+
...
denominator_granular<-deno_dataframe%>%
  ...
  select(location_lact_list, `Lactation Group`, breed_simple, ct_animals, contains('date'))

write_parquet(denominator_granular, here::here('data/intermediate_files/denominator_calendar_time_custom_breed.parquet'))
...
fxn_create_deno<-function(time_period){
  denominator_granular%>%
    group_by(location_lact_list, `Lactation Group`, breed_simple, {{time_period}})%>% #add custom grouping variable here
...
denominator_time_period<-bind_rows(denominator_year, denominator_season, denominator_month, denominator_week)%>%
  select(location_lact_list, `Lactation Group`, breed_simple, contains('time_period'), contains('animal'), contains('floordate'))%>% #add custom grouping variable here
...
write_parquet(denominator_time_period, here::here('data/intermediate_files/denominator_time_period_custom_breed.parquet'))
...
for (i in seq_along(calendar$date_calendar)){
  ...
  df3<-df%>%
    ...
    group_by(location_lact_list, lact_group, breed_simple, date_ref, dim_categories) |> #add custom grouping variable here
  ...
  deno_dataframe_phase <-bind_rows(deno_dataframe_phase, df3)
  
  print(calendar$date_calendar[[i]])
    
}

write_parquet(deno_dataframe_phase, here::here('data/intermediate_files/denominator_phase_time_custom_breed.parquet'))
...
list_custom_var<-sort(unique(deno_dataframe_phase$breed_simple)) #add custom grouping variable here


for (i in seq_along(list_custom_var)){
p<-ggplot(deno_dataframe_phase%>%filter(breed_simple %in% list_custom_var[[i]]))+
  geom_point(aes(x = date_ref, y = ct_animal_lacts, color = dim_categories, shape = breed_simple))+
  ylim(c(0, NA))+
  facet_grid(location_lact_list~`Lactation Group`)+
  labs(title = paste0('Breed = ', list_custom_var[[i]]))
print(p)
}
