...
events <- events |>
  ...
  #events3 <-events2|>
  #fix na values-------------
  mutate(technician = Technician, eid = EID, pen_event = PEN) |>
  ...
  mutate(id = ID, herdid = HERDID) %>%
  mutate(
    animal_lact_status = case_when(
...
write_parquet(
  qc_animal_enrollment,
  "data/qc_files/qc_animal_enrollment.parquet"
)
...
template_event_details <- events |>
  ...
  ungroup()
write_csv(
  template_event_details,
  'data/template_files/template_event_details.csv'
)
